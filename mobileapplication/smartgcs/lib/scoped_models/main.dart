...
import 'package:smartgcs/models/offering.dart';
...
class ConnectedModel extends Model {
  ...
  String _dbUrl = "https://console.firebase.google.com/";
  int _httpTimeout = 5;
...
class UserModel extends ConnectedModel {
  String _apiKey = 'AIzaSyBZ3CUFE9PFXcMLe0VvJeOgfvILx6JMEkA';
  ...
  Future<Map<String, dynamic>> signup(String email, String password,AuthService _auth,
    ...
    String message = 'Authentication Success';
     if (vendor == null) {
     dynamic result= await _auth.registerWithEmailPassword(email,password,client: client);
     if(result==null){
         success = false;
     }
     else{
       IDCardClass.userid=result.uid;
       IDCardClass.type='client';
     }
     }
     else{
     dynamic result= await _auth.registerWithEmailPassword(email,password,vendor: vendor);
     if(result==null){
         success = false;
     }
     else{
        IDCardClass.userid=result.uid;
        IDCardClass.type='vendor';
     }
     }
    ...
    //if (responseData.containsKey('idToken')) {
      ...
      if (!success) message = 'Failed to upload user data with this email';
    // } else {
  ...
  /// Log user in
  Future<Map<String, dynamic>> login(String email, String password,AuthService _auth, UserType userType) async {
    toggleLoading(true);
   ...
   // String id=result.uid;
    ...
    final snapShot = await Firestore.instance.collection('users').document(result.uid).get();
    if (snapShot.exists){
        //it exists
         IDCardClass.userid=result.uid;
    ...
    final snapShot = await Firestore.instance.collection('drivers').document(result.uid).get();
  ...
  //  if (!success) {message = "User does not exist, Please sign up";
  //        code = 0;
  //       }
    // if(result==null){
    //      success = false;
...
class OfferingModel extends ConnectedModel {
  ...
  Future<List<Vendor>> fetchClosestVendors() async{
    ...
    ///  'pos': _client.pos////////////////////////////////////////////close map to implement////////////////////////////////////////
    }),
    headers: {
